                    ...
                    <form action="<?= base_url() ?>home/<?= $a ?>" method="post">
                        <div class="col-md-3">
                            ...
                            <select id="year_id" name="year_id"
                                ...
                                <?php foreach ($year_medium_info as $u) { ?>
                                    <option value="<?= $u->year_id ?>"><?= $u->year_id ?></option>
                                <?php } ?>
                            </select>
                        </div>
                        <div class="col-md-3">
                            <label for="medium" style="font-size: medium">Medium</label>
                            <select id="medium" name="medium"
                                    class="select2-container form-control select2">
                                <option value="0">Select A Medium</option>
                                <?php foreach ($year_medium_info as $u) { ?>
                                    <option value="<?= $u->medium ?>"><?= $u->medium ?></option>
                                <?php } ?>
                            </select>
                        </div>
                        <div class="col-md-3">
                        ...
                        <div class="col-md-1">
                            <button class="button-color form-control" type="submit" name='submit' value='1'
                                    style='margin-top: 60%;margin-right: 200%;width: 65px;'>Submit
                            </button>
                        </div>
                    </form>
                </div>
                        ...
                        <table class="table table-bordered table-striped mb-none custom-table" id="datatable-default">
                            ...
                            <?php if ($content == null) {
                                ...
                                <?php foreach ($content as $row) { ?>
                                    <tr class="gradeX">
                                        <td>
                                            <a href="<?= base_url() ?>assets/downloads/class_routine/<?= $row->class_routine ?>">Download
                                                Class Routine for <?= ' ' . $row->batch_name ?></a></td>
                                    </tr>
                                <?php }
                            } ?>
                            </tbody>
